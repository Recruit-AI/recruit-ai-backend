...
let main_info = {cpa_pantheon_id: 0, cpa_category_id: 0}
let main_object = {}

const user_cred = {username: "kp_creator", password: "test", user_email: "kp_creator"}
let user_token = ""
let user_obj = {}

module.exports = describe("***", () => {
  describe("CategoryToPantheon Tests", () => {
  ...
  it("Create pantheon with auth", async () => {
      ...
      main_info.cpa_pantheon_id = pantheon_id
  ...
  it("Create category with auth", async () => {
      ...
      main_info.cpa_category_id = category_id
      expect(response.status).toBe(expectedStatusCode);
  })
  ...
  it("Create kind_pantheon", async () => {
      const expectedStatusCode = 201;
      const response = await request(server).post('/api/categories/pantheons').send(main_info).set("Authorization", user_token);
  ...
  it("Edit kind_pantheon", async () => {
      const expectedStatusCode = 200;
      const response = await request(server).put(`/api/categories/pantheons/${main_object.category_pantheon_id}`).send(main_info).set("Authorization", user_token);
      expect(response.status).toBe(expectedStatusCode);
  })
  it("Delete kind_pantheon", async () => {
      const expectedStatusCode = 200;
      const response = await request(server).delete(`/api/categories/pantheons/${main_object.category_pantheon_id}`).set("Authorization", user_token);
      expect(response.status).toBe(expectedStatusCode);
  })
